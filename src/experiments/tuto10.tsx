...
import { vertex_shader_3d_300es } from '../shaders/vertex_shader_3d_300es';
import { resizeCanvasToDisplaySize, createProgram, createShader } from '../utils/webglUtils';
import { setFFigure3D } from '../utils/drawFigures';
...
function setupWebGL(gl: WebGL2RenderingContext, pointerParams: PointerParams): WebGLStuff {
  ...
  var vertexShaderSource = vertex_shader_3d_300es;
...
class Canvas extends React.Component<CanvasProps, CanvasState> {
  ...
  matrixCalculations(gl: WebGL2RenderingContext, matrixLoc: WebGLUniformLocation) {
    ...
    // rotation
    var xRad = degToRad(this.props.xRotation);
    var yRad = degToRad(this.props.yRotation);
    var zRad = degToRad(this.props.zRotation);
    var xRotation = mat4.xRotation(xRad);
    var yRotation = mat4.yRotation(yRad);
    var zRotation = mat4.zRotation(zRad);

    // translation
    var fixTranslationX = gl.canvas.width / gl.canvas.height;  // multiply the translation by the inverse of the scale fix
    var translation = mat4.translation(this.props.translation[0] * fixTranslationX, this.props.translation[1], 0);

    // scaling
    var scaling = mat4.scaling(this.props.scaling[0], this.props.scaling[1], 1);
    //var moveOrigin = mat3.translationMatrix(.06, .04);

    // matrix multiplication
    var matrix = mat4.multiply(translation, zRotation);
    matrix = mat4.multiply(matrix, xRotation);
    matrix = mat4.multiply(matrix, yRotation);
    matrix = mat4.multiply(matrix, scaling);
